...
export default function AdminArtworksReview() {
    ...
    const fetchArtworks = async () => {
      try {
        ...
        for (const docSnap of snapshot.docs) {
          ...
          const artistName = userDoc.exists() ? userDoc.data().name : "לא ידוע";
  ...
  const updateApproval = async (userId, artId, approved) => {
    const userRef = doc(
      db,
      "users",
      userId,
    ...
    await Promise.all([
      updateDoc(userRef, { approved }),
      updateDoc(centralRef, { approved }),
    ]);

    setArtworks((prev) =>
      prev.map((a) => (a.id === artId ? { ...a, approved } : a))
    );
  };
            ...
            <div key={art.id} className="artwork-card">
              ...
              <div className="artwork-info">
                ...
                {art.approved ? (
                  <button
                    className="unapprove-button"
                    onClick={() => updateApproval(art.userId, art.id, false)}
                  >
                    ❌ בטל אישור
                  </button>
                ) : (
                  <button
                    className="approve-button"
                    onClick={() => updateApproval(art.userId, art.id, true)}
                  >
                    ✔️ אשר יצירה
                  </button>
                )}
