...
function Card({
  ...
  return (
    <div className="card-root" style={{ position: "relative" }}>
      ...
      {/* Download buttons on a new row */}
      {(onCreateLabels || onDownloadArtistBios) && (
        <div style={{ marginTop: 8, textAlign: "center", display: 'flex', gap: 8, justifyContent: 'center' }}>
          {onCreateLabels && (
            <button className="card-btn download" style={{ width: "45%" }} onClick={onCreateLabels}>
              הורד תוויות (Word)
            </button>
          )}
          {onDownloadArtistBios && (
            <button className="card-btn download" style={{ width: "45%" }} onClick={onDownloadArtistBios}>
              הורד דפי אמנים (PDF)
            </button>
          )}
        </div>
